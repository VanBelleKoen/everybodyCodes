    ...
    for number in numbers[1:]:
        for segment in spine:
            if ((number < segment["value"] and segment["left"] is None) or
                ...
                segment["left" if number <
                        segment["value"] else "right"] = number
                break
        else:
...
def part2(filepath: str):
    qualities = [get_quality(build_fishbone(numbers))
                 for _, numbers in read_line(filepath)]
...
def part3(filepath: str):
    ...
    checksum = sum(sword_id * (index + 1)
                   for index, (sword_id, _, _) in enumerate(swords))
